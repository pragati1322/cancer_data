...
cancerdata = pd.read_csv(r"C:\Users\mukhr\OneDrive\Desktop\360 Digi\Data science ml\KNN\cancerdata.csv")

# Creating engine which connect to MySQL
user = 'root' # user name
pw = 'Mummypapa1' # password
db = '360db' # database
...
KNN = knn.fit(X_train_arr, Y_train_arr)  # Train the kNN model

# Evaluate the model with train data
pred_train = knn.predict(X_train_arr)  # Predict on train data

pred_train

# Cross table
pd.crosstab(Y_train_arr, pred_train, rownames = ['Actual'], colnames = ['Predictions']) 

print(skmet.accuracy_score(Y_train_arr, pred_train))  # Accuracy measure

# Predict the class on test data
pred = knn.predict(X_test_arr)
pred

# Evaluate the model with test data
print(skmet.accuracy_score(Y_test_arr, pred))
pd.crosstab(Y_test_arr, pred, rownames = ['Actual'], colnames = ['Predictions']) 

cm = skmet.confusion_matrix(Y_test_arr, pred)
...
for i in range(3, 50, 2):
    ...
    neigh.fit(X_train_arr, Y_train_arr)
    train_acc = np.mean(neigh.predict(X_train_arr) == Y_train_arr)
    test_acc = np.mean(neigh.predict(X_test_arr) == Y_test_arr)
...
KNN_new = grid.fit(X_train_arr, Y_train_arr) 
...
# Predict the class on test data
pred = KNN_new.predict(X_test_arr)
pred
